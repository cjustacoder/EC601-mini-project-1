...
def collection(name, number, limit, url, label_all):
...
def add_collection(collec_temp):
    myclient = pymongo.MongoClient("mongodb://localhost:27017/")
    mydb = myclient["mini_project_3_MongoDB"]
    mycol = mydb["customers"]

    mycol.insert_one(collec_temp)
    pass
...
def main():
    ...
    print("All set!")
    jsonout = collection(name, number, limit, url, label)
    add_collection(jsonout)
    # # write tweet objects to JSON
    # file = open('access_data.json', 'w')
    # print("Writing tweet objects to JSON please wait...")
